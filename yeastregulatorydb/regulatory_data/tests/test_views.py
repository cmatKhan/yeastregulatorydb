...
from ..models import Binding, ChrMap, DataSource, Regulator
from .factories import BindingFactory
...
def test_single_binding_upload(cc_datasource: DataSource, regulator: Regulator, chrmap: QuerySet, user: User):
    ...
    with open(file_path, "rb") as file_obj:
        ...
        assert (
            re.match(r"binding\/brent_nf_core_callingcards_dev\/\d+\.qbed\.gz$", Binding.objects.get().file.name)
            is not None
...
def test_expression_single_upload(
    expression_datasource: DataSource, regulator: Regulator, chrmap: QuerySet, user: User
):
    token = Token.objects.get(user=user)
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION="Token " + token.key)
    ...
    data = model_to_dict_select(BindingFactory.build())
    # set path to test data and check that it exists
    file_path = os.path.join(os.path.dirname(__file__), "test_data", "expression/hap5_expr17_chr1_ucsc.bed.gz")
    assert os.path.exists(file_path), f"path: {file_path}"
    ...
    with open(file_path, "rb") as file_obj:
        file_content = file_obj.read()
        # Create a SimpleUploadedFile instance
        upload_file = SimpleUploadedFile("hap5_expr17_chrI.bed.gz", file_content, content_type="application/gzip")
        data["file"] = upload_file
        data["source"] = expression_datasource.id
        data["regulator"] = regulator.id
        ...
        assert Binding.objects.count() == 1
        assert (
            re.match(r"binding\/brent_nf_core_expression_dev\/\d+\.bed\.gz$", Binding.objects.get().file.name)
            is not None
        ), Binding.objects.get().file.name


@pytest.mark.django_db
def test_expression_bulk_upload(expression_datasource: DataSource, regulator: Regulator, chrmap: QuerySet, user: User):
    token = Token.objects.get(user=user)
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION="Token " + token.key)
    ...
    csv_path = os.path.join(base_path, "binding_bulk_expression_upload.csv")
    assert os.path.exists(csv_path), f"path: {csv_path}"

    expression_file1_path = os.path.join(base_path, "expression/hap5_expr17_chrI.bed.gz")
    assert os.path.exists(expression_file1_path), f"path: {expression_file1_path}"

    expression_file2_path = os.path.join(base_path, "expression/hap5_expr18_chrI.bed.gz")
    assert os.path.exists(expression_file2_path), f"path: {expression_file2_path}"

    csv_handle = open(csv_path, "rb")
    expression_file1_handle = open(expression_file1_path, "rb")
    expression_file2_handle = open(expression_file2_path, "rb")
    data = {
        "csv_file": SimpleUploadedFile("bulk_upload.csv", csv_handle.read(), content_type="text/csv"),
        "files": [
            SimpleUploadedFile(
                "hap5_expr17_chrI.bed.gz", expression_file1_handle.read(), content_type="application/gzip"
            ),
            SimpleUploadedFile(
                "hap5_expr18_chrI.bed.gz", expression_file2_handle.read(), content_type="application/gzip"
            ),
        ],
    }
    ...
    assert Binding.objects.count() == 2, Binding.objects.count()
    csv_handle.close()
    expression_file1_handle.close()
    expression_file2_handle.close()
