...
from callingcardstools.PeakCalling.yeast.read_in_data import read_in_chrmap, read_in_experiment_data
...
def promoter_significance_combined_task(
    self, regulator_id: int, user_id: int, output_fileformat: str, data_usable: str = "pass", **kwargs
) -> list:
    ...
    try:
        binding_records = Binding.objects.filter(
            bindingmanualqc__data_usable=data_usable, regulator_id=regulator_id, datasource__name="callingcards"
        )
    except Binding.DoesNotExist:
        raise ValueError(
            f'Binding records with regulator_id {regulator_id} and data_usable "{data_usable}" do not exist'
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # iterate over all Binding records and extract the files to the tmpdir
        binding_filepath_list = [
            extract_file_from_storage(binding_record.file, tmpdir) for binding_record in binding_records
        ]
        ...
        for promoter_record in promoterset_objects_iterator:
            ...
            for background_record in background_objects_iterator:
                ...
                result = callingcards_promoter_sig(
                    experiment_data_path=binding_filepath_list,
        ...
        for res_obj in result_list:
            ...
            upload_data = {
                "binding": [binding_record.id for binding_record in binding_records],
